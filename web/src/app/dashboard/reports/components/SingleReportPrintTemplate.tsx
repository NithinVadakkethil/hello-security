...
export default function SingleReportPrintTemplate({
  ...
  const guardName = emp
    ...
    : mgrEmp
      ...
      : report.managerUser?.email ||
        (report.user
          ? `${report.user.firstName || ''} ${report.user.lastName || ''}`.trim() ||
            report.user.name
          : '') ||
        (report.officerName && report.officerName !== 'Inspector'
          ? report.officerName
          : '—');
  ...
  const officerRole =
    ...
    mgrEmp?.role ||
    report.employeeRole ||
    (report.managerUserId ? 'MANAGER' : 'SECURITY');
  ...
  const notCompletedTasksCount = Math.max(
    allTasksTotal - completedTasksCount,
    0,
  );
  ...
  const verificationPillClass =
    getVerificationStatusPillClass(verificationStatus);
  ...
  const printContent = (
    <div className="audit-report-print-root">
      ...
      <div className="rpt-summary-grid">
        ...
        <div className="rpt-kpi-box">
          <div className="rpt-kpi-label">Not Scanned Gates</div>
          <div className="rpt-kpi-val">{pendingGatesCount}</div>
        </div>
        <div className="rpt-kpi-box">
        ...
        <div className="rpt-kpi-box">
          <div className="rpt-kpi-label">Completed Tasks</div>
          <div className="rpt-kpi-val green">{completedTasksCount}</div>
        </div>
        <div className="rpt-kpi-box">
          <div className="rpt-kpi-label">Not Completed Tasks</div>
          <div className="rpt-kpi-val red">{notCompletedTasksCount}</div>
        </div>
      </div>
        ...
        >
          ...
          {snags.map((snag: any, idx: number) => {
            ...
            const latestAssignment =
              Array.isArray(snag.assignments) && snag.assignments.length > 0
                ? snag.assignments[0]
                : null;
            ...
            const assignedTechEmpId =
              latestAssignment?.assignedTo?.employee?.employeeNumber || '—';
            const assignedTechRole = getRoleLabel(
              latestAssignment?.assignedTo?.role ||
                latestAssignment?.assignedTo?.employee?.role ||
                'TECHNICIAN',
            );
            const assignedByAdmin = latestAssignment?.assignedBy?.employee
              ...
              : 'Admin';
            const assignedAtTime = latestAssignment?.createdAt
              ? fmtDateTime(latestAssignment.createdAt)
              : null;
            ...
            const completionEntry = Array.isArray(snag.history)
              ? snag.history.find(
                  (h: any) =>
                    h.action === 'RESOLVED' || h.newState === 'RESOLVED',
                )
              : null;
            ...
            const completedTechEmpId =
              techUser?.employee?.employeeNumber || assignedTechEmpId;
            ...
            const techRemarks =
              completionEntry?.notes ||
              (snag.status === 'RESOLVED'
                ? 'Technician completed repair job and verified checkpoint QR.'
                : null);
            ...
            return (
              ...
              >
                  ...
                  <div>
                    <span
                      style={{
                        ...
                        borderRadius: '4px',
                        backgroundColor:
                          snag.priority === 'HIGH' ? '#fee2e2' : '#fef3c7',
                ...
                >
                  ...
                  >
                    <div>
                      Reported By: <strong>{reporterName}</strong> (ID:{' '}
                      {reporterEmpId})
                    </div>
                    <div>
                      Reported At:{' '}
                      <strong>{fmtDateTime(snag.createdAt)}</strong>
                    </div>
                    <div>
                      Category: <strong>{snag.category}</strong>{' '}
                      {snag.subCategory ? `• ${snag.subCategory}` : ''}
                    </div>
                    <div>
                      Initial Status: <strong>{snag.status}</strong>
                    </div>
                  </div>
                  <div
                    style={{
                ...
                {latestAssignment ? (
                  ...
                  >
                    ...
                    >
                      <div>
                        Assigned To: <strong>{assignedTechName}</strong> (ID:{' '}
                        {assignedTechEmpId})
                      </div>
                      <div>
                        Assigned Role: <strong>{assignedTechRole}</strong>
                      </div>
                      <div>
                        Assigned By: <strong>{assignedByAdmin}</strong>
                      </div>
                      <div>
                        Assigned At: <strong>{assignedAtTime}</strong>
                      </div>
                      <div>
                        Assignment Status:{' '}
                        <strong>{latestAssignment.status || 'ASSIGNED'}</strong>
                      </div>
                    </div>
                  </div>
                ) : (
                ...
                >
                  ...
                  {repairResult !== 'PENDING' ? (
                    <>
                      ...
                      >
                        <div>
                          Technician: <strong>{completedTechName}</strong> (ID:{' '}
                          {completedTechEmpId})
                        </div>
                        <div>
                          Verified At:{' '}
                          <strong>
                            {fmtDateTime(
                              completionEntry?.createdAt || snag.updatedAt,
                            )}
                          </strong>
                        </div>
                      </div>
                      ...
                      <div style={{ marginTop: '6px' }}>
                        ...
                        {afterImages.length > 0 ? (
                          ...
                          >
                            No after-repair photo attached
                          </div>
                        )}
                      </div>
                    </>
                    ...
                    >
                      Repair Verification: PENDING (Technician repair
