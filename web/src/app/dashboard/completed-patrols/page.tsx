...
function CompletedPatrolsContent() {
  ...
  // Query Completed Patrol Sessions with backend pagination & search
  const { data: responseRes, isLoading } = useQuery<{
    ...
    queryKey: ['patrol-sessions-completed', page, search],
    queryFn: () =>
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: PatrolSession) => {
        ...
        currentParams.set('from', 'completed');
        const queryStr = currentParams.toString();
        const detailsHref = `/dashboard/patrol-sessions/${row.id}?${queryStr}`;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      >
        <SearchBar
          ...
          placeholder="Search historical logs by code, guard name or site..."
        />
      </div>
      ...
      <DataTable
        ...
        data={sessions}
        isLoading={isLoading}
        emptyMessage="No historical patrol records found."
      />
...
export default function CompletedPatrolsPage() {
  return (
    <Suspense fallback={<div style={{ padding: '24px', color: '#64748b' }}>Loading Completed Patrols...</div>}>
      <CompletedPatrolsContent />
    </Suspense>
  );
}
