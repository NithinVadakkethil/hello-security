...
export default function GateSubTasksModal({
  ...
  const [editingSubTask, setEditingSubTask] = useState<GateSubTask | null>(
    null,
  );
  ...
  const createMutation = useMutation({
    mutationFn: (values: SubTaskValues) =>
      apiClient.post(`/gates/${gateId}/sub-tasks`, {
        ...values,
        role: selectedRole,
      }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      apiClient.delete(`/gates/${gateId}/sub-tasks/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ['sub-tasks', gateId, selectedRole],
      });
  ...
  const reorderMutation = useMutation({
    mutationFn: (items: Array<{ id: string; displayOrder: number }>) =>
      apiClient.patch(`/gates/${gateId}/sub-tasks/reorder`, {
        subTasks: items,
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({
  ...
  const activeRoleObj =
    ROLE_OPTIONS.find((r) => r.key === selectedRole) || ROLE_OPTIONS[0];

  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Checkpoint Verification Sub-Tasks: ${gateName}`}
    >
      <div
        style={{
          display: 'flex',
      ...
      >
        ...
        {isFormOpen && (
          ...
          >
            ...
            >
              {editingSubTask
                ? `Edit ${activeRoleObj.label} Task`
                : `Add New ${activeRoleObj.label} Task`}
            </h4>
            <form
                ...
                >
                  <input
                    ...
                    style={{
                      width: '16px',
                      height: '16px',
                      accentColor: activeRoleObj.color,
                    }}
                  />
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    borderColor: activeRoleObj.color,
                  }}
                >
                  {createMutation.isPending || updateMutation.isPending
                    ? 'Saving...'
        ...
        {isLoading ? (
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: '0.82rem' }}>
              Click <b>&quot;Add {activeRoleObj.label} Task&quot;</b> above to
              create role-specific verification questions.
            </p>
          </div>
        ) : (
          <div
                      ...
                      {task.isRequired ? (
                        ...
                        <span
                          style={{
                            ...
                            padding: '2px 8px',
                            borderRadius: '12px',
                            backgroundColor: 'var(--chip-bg, #333)',
                            color: 'var(--text-muted)',
                            fontWeight: 600,
                          }}
                        >
                ...
                >
                  <button
                    ...
                    style={{
                      ...
                      background: 'none',
                      cursor: 'pointer',
                      color: task.isActive ? '#10b981' : '#64748b',
                    }}
                  >
                    {task.isActive ? (
                      <ToggleRight size={22} />
